...
class Population {
  ...
  //find and return the genes of best rocket of the population
  PVector[] getBestGenes(){
    float record = 0;
    PVector[] best_genes = new PVector[lifetime];
    for (int i = 0; i < population.length; i++) {
       if(population[i].getFitness() > record) {
         best_genes = population[i].getDNA().getGenes();
         println(best_genes[0].x);
       }
    }
    return best_genes;
  }
